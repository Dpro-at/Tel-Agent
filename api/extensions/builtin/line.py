"""LINE — the customer's own official account, shipped official (D-032).

The transport lives in `api/channels/line.py` and receives through the generic public
...
logger = logging.getLogger("api.extensions.line")
...
MANIFEST = {
    ...
    "origin": "official",
    "category": "channels",
    "description": "Answers people who message your LINE official account, and group "
    "chats that mention it.",
...
async def _on_message(**payload: Any) -> None:
    ...
    logger.debug(
        "line saw a message", extra={"conversation_id": payload.get("conversation_id")}
    )
